...
## 2026-04-24 修复：开仓成功判断 + 冷却时间延长

### 发现的问题

**问题：开仓成功判断与平仓相同**
- **现象**：BSBUSDT 00:28 冷却到期后，信号满足 → 尝试开仓 → API返回 `{'clientOid': ..., 'orderId': ...}` 但无 code → 被判为失败
- **根因**：只检查 `code == '00000' or code == '0'`，但 Bitget 部分成功响应无 code 字段
- **后果**：开仓API失败，但交易所实际成交了

**冷却时间过短**
- 原设置：600秒（10分钟）
- 问题：冷却到期后，信号仍满足，立刻重复开仓
- 改为：900秒（15分钟）

### 修复内容

#### 修复1：开仓成功判断（futures_trader.py）
```python
# 改前
...
| 文件 | 改前 | 改后 |
|------|------|------|
| futures_trader.py | 600秒 | 900秒 |
| trailing_stop_v2.py | 600秒 | 900秒 |

### 重启记录

| 时间 | 操作 | PID |
|------|------|-----|
| 2026-04-24 00:36 | futures_trader.py 重启 | - |
| 2026-04-24 00:36 | trailing_stop_v2.py 重启 | - |

### Git 提交

```
fix: 开仓成功判断 — 有orderId即为成功，兼容无code响应
fix: 冷却时间从600秒延长至900秒
```
